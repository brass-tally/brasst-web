-- NOTE: the select policy below was wide open, so the whole waitlist was
-- readable by anyone holding the publishable key. It is left here because this
-- migration has already run in production and applied migrations are not
-- rewritten. 0019_harden_beta_rls.sql replaces it. If you are building a fresh
-- environment, both run in order and you end up in the right place.
